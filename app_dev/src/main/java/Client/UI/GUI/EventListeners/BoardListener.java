...
public class BoardListener extends MouseAdapter {
    ...
    public BoardListener(GUI gui, PlacementArea board){
        this.gui = gui;
        this.board = board;
    }
    @Override
    public void mouseClicked(MouseEvent e) {
        ...
        if(gui.starterSelected){
            ...
            if(xRel / (board.getCardLength()/2) == 0 && yRel / (board.getCardHeight()/2) == 0){
                ...
                gui.starterSelected = false;
                System.out.println("cliccata");
            }
        }
        if(gui.cardSelected && gui.getCurrentDisposition().equals(gui.getView().getPlayerId())){
